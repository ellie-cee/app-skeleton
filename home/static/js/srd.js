     ...
     showUserErrors(data,queryRoot) {
        ...
        if (data[queryRoot].userErrors && data[queryRoot].userErrors.length>0) {
            data[queryRoot].userErrors.forEach(error=>{
                let field = '';
                if (error.fields!=null && error.fields.length>1) {
        ...
        return this.showErrorModal(messages);
    }
...
class FunctionAwareCRUD extends BaseCRUD {
        ...
        fetch(`/api/functions/list/${this.function_type()}`)
            .then(response=>response.json()).then(q=>{
                ...
                if (q.data.shopifyFunctions.nodes && q.data.shopifyFunctions.nodes.length>0) {
                    ...
                    `);
                    document.addEventListener("srd:modal:closed",event=>history.back())
                    return;
                }
                if (this.config.id) {
                    this.load(this.config.id)
    ...
    functionConfig() {
        ...
        let config = JSON.parse(document.querySelector("#config-data").textContent)
        console.error(config)    
        let conf = null;
        Array.from(config).forEach(node=>{
            console.error(node)
            console.error(node.functionId,this.config.selectedFunction)
            if (node.functionId==this.config.selectedFunction) {
                conf = node
            }
        })
