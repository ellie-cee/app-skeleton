class WebhooksCRUD extends GenericCRUD {
    ...
    objectFromQuery(q) {
        ...
        return q.data.webhookSubscription
    }
    defaultObject() {
        return  {
            "id": null,
    ...
    urls(action) {
        return {
            "load":`/api/webhooks/get/${this.config.id}`,
            "create":"/api/webhooks/new",
            "update":`/api/webhooks/update/${this.config.id}`,
            "delete":`/api/webhooks/delete/${this.config.id}`
        }[action];
     }
     processReturn(action,data) {
        ...
        switch(action) {
            case "delete":
                location.href="/webhooks";
                break;
            case "create":
                let details = data.webhookSubscriptionCreate;
                this.object.id = this.gid2id(details.webhookSubscription.id)
     ...
     innerHTML() {
        return `
            ${new JSONSelectField({"topic":this.object.topic},{"name":"topic","label":"Webhook Topic","default":"",options:this.topics()}).render()}
            ${new JSONText({"url":this.object.endpoint.callbackUrl.replace(this.config.baseUrl,"")},{"name":"url","label":"Webhook Path","default":"webhooks/default"}).render()}
        `
     }
}  
